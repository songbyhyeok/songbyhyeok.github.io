...
# Intro
최근 나는 프로젝트를 만들겠다고 다짐만 수없이 해왔던 것 같다. 시간을 어떻게든 확보하겠다고 말은 하지만, 결국 지금 하는 일들을 줄이거나 중단해야 한다는 사실을 깨닫고 나서야 비로소 시작할 수 있었다.  

나는 원래 글을 쓰거나 기록하는 것과 거리가 먼 사람이었다. 그래서 '되는 대로 만들고 나중에 작성하면 되지'라고 미루기를 수십 번… 결국 만들어진 제품은 설명서가 없어 가치를 부여할 수가 없었다.  

잘 만들어진 프로젝트는 설명서만 읽어도 그 가치를 증명할 수 있다는 생각이 든다. 나도 그래서 개발 일지를 기록하면서 프로젝트의 가치를 높여보고 싶었다.
<br>
