...
그렇다면 r-value 전달을 하면 어떻게 될까? r-value 역시 전달될 때 Copy Elision에 의해 0 copy가 되고 이후 move 키워드를 사용해 다시 소유권 이전을 시키기 때문에 copy가 절대 발생하지 않게 된다. 따라서 **pass by value 전달 방식을 사용하는 것이 좋다.**
