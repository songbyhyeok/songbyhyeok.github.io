...
# HandlerInterceptor
클라이언트의 요청을 처리할 핸들러가 실행되기 전에 **(Interceptor 하여)**, 특정 작업을 수행할 수 있도록 서버에 제공하는 인터페이스다. 주된 목적은 반복적 코드를 통합하여, 이를 재사용할 수 있게 하는 것이다.  
...
사전상 "가로채는" 라는 뜻을 가진 인터셉터는 처리 주기를 변경하거나 증강하는 방법을 제공하고자 할 때 사용되는 디자인패턴을 말하며,  
변경 사항이 투명 하고 자동으로 사용된다는 이점 때문에 프레임워크에서 일반적으로 사용한다.  

## 특징
구조 특성상 많은 종류의 전.후 핸들러를 요청 핸들러 사이에 서버가 개입할 수 있어, 요청이 컨트롤러의 핸들러 메서드에 도달하기 전 또는 응답이 클라이언트로 돌아가기 전에 추가적인 작업을 할 수 있다. 그리고 미리 필요한 설정이나 특정 로직을 추가 및 변경할 수 있기 때문에, 핸들러의 실행 환경을 제공한다. 이를 통해 여러 핸들러에서 공통적으로 필요한 로직(예: 인증, 권한 체크, 로깅, 세션 처리 등)을 각각의 핸들러에서 직접 작성하는 대신, 인터셉터를 통해 공통적으로 처리할 수 있기 때문에 코드를 간결하게 하고 **재사용성**을 높이며, **유지보수**를 가능하게 만든다. 마지막으로 다양한 인터셉터를 조합하여 요청 처리 흐름을 세밀하게 제어하기 때문에 **유연성**을 제공한다.

### 주의
Spring 5.3 이전까지는 HandlerInterceptorAdapter 클래스를 사용하여 간편하게 인터셉터를 생성할 수 있었지만, 이후부터는 사용 중지(예정) 되었기 때문에, HandlerInterceptor 인터페이스를 직접 구현하는 방식을 권장하고 있다.  
...
1번과 2번 사이, 즉 요청 핸들러 호출 이전에 **HandlerInterceptor**는 가로채서 먼저 특정 작업을 수행한다. 이를 **preHandle API** 호출을 통해 수행하게 되며, 반환 값에 따라 요청 핸들러 호출 처리 과정과 이후에도 인터셉터가 개입할 수 있는 구조를 제공한다.  
...
여기에서 요청을 가로채고 처리할 수 있습니다. 예를 들어, 인증 체크를 할 수 있습니다.
return true를 반환하면 요청이 계속 처리되고, false를 반환하면 요청 처리가 중단됩니다.

* **postHandle**:
요청이 컨트롤러에서 처리된 후, 뷰가 렌더링되기 전에 호출됩니다.
예를 들어, 요청 처리 후 응답에 대해 추가적인 작업을 할 수 있습니다. (예: 추가 데이터 수정)

* **afterCompletion**:
요청 처리 후, 뷰 렌더링이 끝난 후 호출됩니다.
응답이 클라이언트로 전송된 후, 로그를 기록하거나 리소스를 해제하는 등의 작업을 할 수 있습니다.
...
체인(Chain)은 여러 개의 개별적인 작업이나 요소들이 연결되어 하나의 흐름을 형성하는 구조를 말하는데, 이를 여러 개의 인터셉터를 순차적으로 연결시켜, 요청 처리 전후에 실행되는 일련의 단계를 형성할 수 있다.
```
...
요청을 처리하는 핸들러가 별도의 스레드에서 실행되는데, 이 스레드는 요청 처리를 담당하는 동시에 메인 스레드는 요청을 완료하지 않고 빠르게 반환될 가능성이 높다. 따라서 비동기 요청에서는 렌더링(view rendering)이나 후처리(postHandle, afterCompletion 콜백) 
메서드들이 즉시 호출되지 않고, 비동기 처리가 완료된 후에 요청이 다시 반환되어 모델을 렌더링하고 최종적으로 응답을 생성하는 과정이 계속 처리된다. 이때, 비동기 요청의 후처리 단계인 postHandle과 afterCompletion 메서드들이 호출된다. 즉, 비동기 요청이 완료된 후 이 메서드들이 호출되어 후처리가 이루어지게 된다.
