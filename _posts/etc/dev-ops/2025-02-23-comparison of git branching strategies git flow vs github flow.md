...
# 소개
깃 브랜치란 여러 개발자들과 함께 프로젝트를 진행할 때, Git의 브랜치를 활용해 효율적으로 작업을 분담하고 관리하는 방법을 말한다. 여기서 'workflow'란 '작업 절차를 통한 정보나 업무의 흐름'을 의미하며, 즉, 작업 흐름을 뜻한다.  

## 브랜치 부재
깃 브랜치를 사용하지 않고 프로젝트를 진행했다고 가정하자.  

우선, 여러 개발자가 동시에 프로젝트를 진행하는 과정에서 코드 충돌과 작업의 혼란이 빈번하게 발생했을 것이다. 각 개발자가 독립적인 작업을 할 공간이 부족해 버전 관리가 어려워졌고, 그로 인해 기능 개발 중 안정성 문제나 배포 오류가 자주 발생했을 것이다. 또한, 협업의 비효율성으로 팀워크가 방해받고, 테스트와 피드백이 제대로 이루어지지 않아 프로젝트가 매우 복잡하고 비효율적으로 진행되었을 것이다.  

## 브랜치를 통한 원활한 협업과 관리
그렇다면, 브랜치를 사용한다면 어떻게 될까?  

동시 작업을 할 수 있다. 여러 개발자가 병렬로 작업할 수 있어 서로의 작업에 영향을 미치지 않는다.  

독립적 작업이 가능하다. 각 개발자는 독립적인 브랜치에서 작업하므로, 다른 개발자의 작업에 방해받지 않는다.  

기능, 이슈 대응이 가능하다. 기능 개발이나 버그 수정을 브랜치 단위로 관리하여 작업 진행 상황을 명확하게 추적할 수 있다.  

작업 추적을 통해 각 브랜치별로 커밋 내역을 확인하여 어떤 작업이 이루어졌는지 쉽게 파악할 수 있다.  

배포 안정성과 관리 유연성, 새로운 기능을 추가하거나 실험적인 작업을 메인 코드베이스에 영향을 주지 않고 유연하게 관리할 수 있으며, 기능이 완성된 후 메인 브랜치에 병합하여 안정적인 상태로 배포할 수 있다.  

## 브랜치 종류
깃 브랜치 종류 중 대표적으로 사용되는 방법은 Git Flow와 GitHub Flow이다.   
이 두 가지 모델에 대해서 중점적으로 알아보려고 한다.
...
설명만 들었을 때, 굉장히 좋은 모델이라고 생각이 들겠지만, 하지만 현재 트렁크 기반 워크플로우의 등장으로 CI/CD와 함께 사용하기엔 어려움이 많다.  
...
그럼에도 Git Flow는 정해진 출시 주기나 지속적 배포(Continuous Delivery) 프로세스에서 여전히 유용하다. 따라서 이 모델에 대해 알아보는 것이 중요하다.  
...
* release
  * 다음 출시 버전을 준비하는 브랜치로, 최종 테스트 및 버그 수정 작업이 진행되며, 완료되면 master와 develop에 병합   
* hotfix
...
master 브랜치는 항상 배포 가능한 상태만을 관리하는 브랜치를 말한다. 그리고 develop 브랜치는 다음 배포를 위한 개발이 이루어지는 브랜치로, 주로 기능들이 통합되는 역할을 한다. 평소에는 개발자들이 이 브랜치를 기반으로 작업을 진행하며, 새로운 기능이나 수정 사항을 반영한다.  
<br>
...
### 흐름
개발자는 신규 기능 개발을 위해 develop 브랜치를 기준으로 feature 브랜치를 생성하여 작업을 진행한다.  

작업이 완료된 feature 브랜치는 develop 브랜치에 Merge 한다. 구체적으로 PR을 통해 작업 내용을 리뷰받고, 리뷰가 완료된 후 PR을 병합하는 방식으로 진행된다.  

다음 출시 버전을 준비하기 위해, develop 브랜치에서 release 브랜치를 생성하여 배포 준비를 시작한다. 이 과정에서 발견된 버그는 release 브랜치에서 바로 수정한다.  

충분한 테스트 후, 일정한 주기(일반적으로 배포하려는 버전 단위)로 release 브랜치를 master 브랜치에 병합하여 제품을 출시한다.  

상용 배포 후, release 브랜치에서 미처 발견되지 않은 새로운 버그들은 hotfix 브랜치에 바로 반영하여 신속하게 수정 작업을 진행한다.  
...
* **GitHub Flow**  
단순하고 빠른 배포를 가능하게 하지만, 테스트와 검증 절차가 없이 master 브랜치로 바로 병합되므로 잠재적인 위험이 존재한다. 그러나 빠르게 반복 가능한 작업을 원할 때 유리하다.  
