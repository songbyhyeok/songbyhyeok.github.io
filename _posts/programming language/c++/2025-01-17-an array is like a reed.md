...
# 소개
코딩을 하던 중 배열을 매개변수로 사용하는 상황이 생겼다. 당연히 배열의 크기를 내부 함수에서 계산해서 처리하면 되겠지 싶었고, 크기를 확인한 후 '?'라는 물음표가 떠올랐다. 예상했던 것과 다른 값이 나왔기 때문이다. 예상대로라면 10000000개의 byte 크기가 나와야 했는데, 결과는 '8'이라는 숫자였다. 왜 이런 결과가 나왔을까?
```Cpp
...
# 배열은 포인터일까?
배열은 상황에 따라 포인터처럼 동작할 수도 있고, 그렇지 않을 수도 있다. 
...
## 배열과 포인터의 차이점
배열 자체는 본질적으로 고정된 크기의 연속된 메모리 공간을 나타내지만, 배열과 포인터는 본질적으로 다르며, 몇 가지 중요한 차이점이 존재한다.
...
ptr[2] = 66;
```
1. array[0] 에서 두 칸을 이동하여 그 값을 666으로 변경한다.
2. ptr[0] 에서 값/주소를 가져오기 위해 주소에 2를 더하고, 역참조를 통해 그 주소에 있는 값에 66을 대입한다.  
<br>
